...
function DashboardPage() {
  ...
  const { isMember, setIsMember, memberBalance, setMemberBalance } = context;
  ...
  const handleDeposit = (e: React.FormEvent) => {
    ...
    setMemberBalance(memberBalance + amount);
    setDepositAmount('');
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-3 bg-white p-2 rounded-xl border border-gray-200 shadow-sm">
          <div className="w-8 h-8 rounded-full bg-coopGold/20 flex items-center justify-center text-sm">
            🦅
          </div>
          <div className="text-left pr-2">
            <span className="block text-xs font-bold text-slate-900 leading-none">Class of '18 Alumnus</span>
            <button 
              onClick={() => setIsMember(!isMember)}
              className="text-[10px] text-coopGreen hover:underline font-semibold block mt-0.5"
            >
              Status: {isMember ? 'Active Member' : 'Guest Account'}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm">
          ...
          <div className="text-2xl font-extrabold text-coopGreen mt-2">
        ...
        <div className="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm">
          ...
          <div className="text-2xl font-extrabold text-coopGold mt-2">₦75,400.00</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* LEFT COLUMN: SIMULATE DEPOSIT */}
        <div className="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm">
          <h3 className="font-serif text-base text-slate-950 font-bold mb-1">Inbound Capital Allocation</h3>
          <p className="text-xs text-gray-400 mb-4">Simulate depositing cash into your secure cooperative wallet.</p>
          ...
          <form onSubmit={handleDeposit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 border border-gray-200 rounded-xl outline-none focus:border-coopGreen focus:ring-2 focus:ring-coopGreen/10 bg-white text-sm transition text-slate-900"
              />
            </div>
            <button
              type="submit"
              className="w-full bg-coopGreen text-white text-xs font-bold py-2.5 rounded-xl hover:bg-coopGreen-dark transition"
            >
              Deposit Funds
            </button>
          </form>
          ...
          {feedback && (
            <div className="mt-3 p-2.5 bg-green-50 border border-green-200 text-green-800 text-xs rounded-xl">
              {feedback}
            </div>
          )}
        </div>
